...
//					.toArray();
            ...
            // 로직
            int V = 100;
            // 해당 입력을 통해 인접행렬...! 노드의 갯수 V x V (정보)
            int[][] adj = new int[V][V];
            // 인접행렬은 노드(정점) 사이의 정보를
            // 연결 관계를 직접 adj[u][v] = 1; // u -> v 경로o
            // 정점 u -> 정점 v로 갈 수 있는 페어를 꺼내와야 한다. pairs 로부터
            for (int i = 0; i < pairs.length; i += 2) {
                int u = pairs[i];
                int v = pairs[i + 1];

                adj[u][v] = 1; // u -> v 갈 수 있음 o
            }
    ...
    // (+isArrive 변수 값을 true)
    private static void recursive(final int V, final int[][] adj, int start, int end) {
        // 기저조건 (종료조건)
        ...
        for (int i = 0; i < V; i++) {
            // start -> i 노드로 갈 수 있는가...!
            // adj[start][i] == 1 이면 갈 수 있다!
            // 방문할 수 있는지 여부 확인...!
            if (adj[start][i] == 0 || visited[start][i] == true) {
                continue;
            }

            // i번 노드로 방문을 진행하겠다...!
            // 방문배열에 체크...!
            visited[start][i] = true; // 결정
            recursive(V, adj, i, end);
            visited[start][i] = false; // 복구
        }
    }
